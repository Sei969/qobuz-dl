...
* **Roon-Ready Synchronized Lyrics (Native Qobuz Support):** The engine natively fetches official, 1:1 zero-drift timestamped lyrics directly from Qobuz's CDN, converting them into standard `.lrc` formats on the fly. It intelligently formats and embeds this data directly into the audio files (`[LYRICS]` Vorbis Comments), ensuring Roon natively displays scrolling, karaoke-style lyrics in its "Now Playing" view out-of-the-box. If you prefer a minimalist, clutter-free folder structure, you can disable the generation of external `.lrc` files entirely via CLI (`--no-lrc-files`). Conversely, if you prefer external files without bloating your audio metadata, use the `--no-embed-lyrics` flag (or set `embed_lyrics = false` in your config).
...
* **Automatic Lyrics Fallback & Retroactive Tagger:** If a track lacks official Qobuz lyrics, the engine seamlessly falls back to fetching synchronized (`.lrc`) and unsynchronized lyrics using LRCLIB (with a Genius fallback API). To strictly prevent polluting your library with incorrect lyrics, all fallback API responses are routed through a **Fuzzy Matching Algorithm** (cross-referencing artist and title) before injection. Includes a standalone `lyrics` command to retroactively scan and inject missing lyrics into your existing local library without re-downloading the audio.
...
* **Clean Multithreading UI & Context-Aware Logic:** Intelligently switches to a clutter-free, static logging system displaying precise file sizes (MB) during concurrent downloads to prevent terminal visual glitches. Furthermore, the engine becomes **Context-Aware**: if a borderline lyrics match is found during parallel batch downloading, it automatically skips the interactive `[y/N]` prompt to prevent UI deadlocks and thread stalling, ensuring massive discography downloads remain 100% unattended.
