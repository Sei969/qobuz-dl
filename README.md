...
* **Automatic Lyrics Engine & Fuzzy Security Shield:** Fetches and injects synchronized (`.lrc`) and unsynchronized lyrics using LRCLIB (with a Genius fallback API). To strictly prevent polluting your library with incorrect lyrics from homonymous tracks, the engine routes every API response through a **Fuzzy Matching Algorithm** (cross-referencing artist and title). Perfect matches (>75%) are injected, completely wrong ones (<60%) are blocked, and borderline matches prompt the user for interactive confirmation. Includes a standalone `lyrics` command to retroactively scan and inject missing lyrics into your existing local library without re-downloading the audio.
